...
-- ═══════════════════════════════════════════════════════════
-- 零、OpenClaw 环境适配层（代码量只增不减）
-- ═══════════════════════════════════════════════════════════
-- 尝试从当前脚本路径推导适配层位置
local thisFile = debug.getinfo(1, "S").source:sub(2) -- 去掉开头的 '@'
local thisDir = thisFile:match("^(.*)/[^/]+$") or "."
local adapterFile = thisDir .. "/network/tao/TaoOpenClawAdapter.lua"
local f = io.open(adapterFile, "r")
if f then
    f:close()
    dofile(adapterFile)
else
    -- fallback：若 main.lua 被复制到标准路径，尝试相对加载
    pcall(function() dofile("scripts/network/tao/TaoOpenClawAdapter.lua") end)
end
